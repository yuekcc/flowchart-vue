...
function styleValue(node, name) {
  return (
    node.style.getPropertyValue(name) || defaultView(node).getComputedStyle(node, null).getPropertyValue(name)
  );
}
...
function selection_text(value) {
  return arguments.length
    ? this.each(
        value == null ? textRemove : (typeof value === 'function' ? textFunction : textConstant)(value),
      )
...
function selection_html(value) {
  return arguments.length
    ? this.each(
        value == null ? htmlRemove : (typeof value === 'function' ? htmlFunction : htmlConstant)(value),
      )
    : this.node().innerHTML;
}
function raise() {
...
function select(selector2) {
  return typeof selector2 === 'string'
    ? new Selection([[document.querySelector(selector2)]], [document.documentElement])
...
function DragEvent(
  type,
  { sourceEvent: sourceEvent2, subject, target, identifier, active, x, y, dx, dy, dispatch: dispatch2 },
) {
  ...
  return (
    ...
    createElementBlock(
      'div',
      {
        ...
        onMousemove:
          _cache[0] ||
          (_cache[0] = (...args) => $options.handleChartMouseMove && $options.handleChartMouseMove(...args)),
        ...
        onMousewheel:
          _cache[3] ||
          (_cache[3] = (...args) =>
            $options.handleChartMouseWheel && $options.handleChartMouseWheel(...args)),
      ...
      [
        createElementVNode(
          'span',
          _hoisted_1,
          toDisplayString($data.cursorToChartOffset.x + ', ' + $data.cursorToChartOffset.y),
          1,
        ),
        _hoisted_2,
